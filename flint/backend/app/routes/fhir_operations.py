...
# OID aliases handle ValueSets where a system reference could not be
...
_SYSTEM_DISPLAY_NAMES: Dict[str, str] = {
    ...
    # CDC local code systems — legacy URL forms that may appear in imported data.
    ...
    # OID aliases — urn:oid: passthrough for systems where canonical URL normalisation was not applied
    ...
    # ISO codes — static entries required
...
_SYSTEM_URL_TO_SDO: Dict[str, str] = {
    ...
    "http://www.nlm.nih.gov/research/umls/rxnorm": "rxnorm",
    "https://cts.nlm.nih.gov/fhir":                "vsac",
    # OID aliases — urn:oid: forms for systems where canonical URL normalisation did not apply
    "urn:oid:2.16.840.1.113883.6.1":               "loinc",
...
async def _get_system_name(
    ...
    Resolution order:
      ...
      5. Legacy CDC baseStu3 URL fallback for urn:oid: references — some imported
         CodeSystems use https://phinvads.cdc.gov/baseStu3/CodeSystem/{oid}
         as the URL while compose.include.system stores urn:oid: notation
      6. HL7 terminology URL pattern (v2/v3 table names — fallback when not in DB)
      7. Pattern-based fallback for CDC and other known URL shapes
    ...
    if cs_results:
        ...
        cache[system] = name
        return name
    # For urn:oid: references, also try the legacy CDC baseStu3 URL pattern
    # used by some imported CodeSystems as their canonical URL.
...
def _url_to_display_name(system: str) -> str:
    ...
    Handles common patterns found in imported ValueSets:
    ...
    # Legacy CDC baseStu3 CodeSystem URL: extract OID from path
    ...
    for include in compose.get('include', []):
        ...
        if 'concept' in include:
            ...
            for concept in include['concept']:
                ...
                # the raw Concept Name in display — some importers store
