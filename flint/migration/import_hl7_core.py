...
content="complete" â€” small administrative code systems that are freely
...
def download_package(client: httpx.Client) -> bytes:
    print(f"Downloading hl7.fhir.r4.core from {PACKAGE_URL} â€¦", flush=True)
    ...
    with tarfile.open(fileobj=io.BytesIO(tarball), mode="r:gz") as tf:
        for member in tf.getmembers():
            ...
            if data.get("resourceType") == "CodeSystem":
                # Strip meta â€” lastUpdated causes datetime serialization errors
                data.pop("meta", None)
                # Strip concept-level property arrays â€” they can contain partial
...
def main():
    ...
    print("Extracting CodeSystem resources â€¦", flush=True)
