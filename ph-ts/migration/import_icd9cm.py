...
resource into the local PH-TS server.
...
codes the script iterates over every 3-character prefix (001–999, V01–V91,
E000–E999) — each prefix returns up to 500 matching codes which are then
...
def fetch_prefix(client: httpx.Client, prefix: str) -> list[dict]:
    ...
    try:
        ...
        print(f"  WARNING: prefix {prefix} failed — {e}", flush=True)
        return []
...
def fetch_all_codes(client: httpx.Client) -> list[dict]:
    ...
    for i, prefix in enumerate(prefixes, 1):
        if i % 100 == 0 or i == 1:
            print(f"  Fetching prefix {i}/{total} ({prefix}) — {len(concepts):,} codes so far …", flush=True)
...
def main():
    ...
    print("Fetching ICD-9-CM codes from NLM ClinicalTables …\n", flush=True)
    ...
    if args.dry_run:
        ...
        print(f"DRY RUN — resource written to {out_path} (not imported)")
        return
