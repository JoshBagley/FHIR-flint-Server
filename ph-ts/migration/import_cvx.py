...
import_cvx.py
============
Imports CDC CVX (Vaccine Administered) codes as a FHIR R4 CodeSystem into PH-TS.

Primary source: local CDC Excel file (docs/cvx_codes/web_cvx.xlsx)
Fallback source: NLM ClinicalTables API (less complete — no status, notes, or non-vaccine flag)
...
FHIR concept properties defined on the CodeSystem:
  - status     (code)    — Active | Inactive | Non-US | Never Active
  - nonVaccine (boolean) — true if this is a placeholder/non-vaccine code
...
def main():
    ...
    # Fetch concepts
    if args.source == "xlsx":
        print(f"Reading CVX codes from {XLSX_PATH.name}…")
        concepts = fetch_from_xlsx()
    else:
        print("Fetching CVX codes from NLM ClinicalTables…")
    ...
    with httpx.Client() as client:
        ...
        if existing_id:
            print(f"\nExisting CVX CodeSystem found (id: {existing_id}) — updating via PUT…")
            ...
            print(f"\nNo existing CVX CodeSystem found — creating via POST…")
            resp = client.post(
                f"{target}/CodeSystem",
                json=resource,
                headers={"Content-Type": "application/fhir+json", "Accept": "application/fhir+json"},
    ...
    if resp.status_code in (200, 201):
        print(f"  + {action} — id: {resp.json().get('id', '?')} ({len(concepts)} concepts)")
    else:
