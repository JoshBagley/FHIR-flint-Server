...
imports every HL7 v2 table CodeSystem resource into the local PH-TS server.
...
def download_package(client: httpx.Client, version: str) -> bytes:
    ...
    print(f"Downloading hl7.terminology.r4 v{version} from {url} …", flush=True)
    print("  (This package is ~80-120 MB — please wait)", flush=True)
...
def extract_v2_code_systems(tarball: bytes) -> list[dict]:
    ...
    with tarfile.open(fileobj=io.BytesIO(tarball), mode="r:gz") as tf:
        ...
        print(f"  Scanning {len(members)} files in package …", flush=True)
...
def post_resource(client: httpx.Client, base_url: str, resource: dict) -> str:
    ...
    resource_copy["extension"] = [
        *[e for e in resource_copy.get("extension", []) if e.get("url") != "http://phts.local/StructureDefinition/source"],
        {"url": "http://phts.local/StructureDefinition/source", "valueCode": "hl7v2"},
    ]
    try:
        resp = client.post(
            api_url,
...
def main():
    parser = argparse.ArgumentParser(description="Import HL7 v2 table CodeSystems into PH-TS")
    ...
    print("\nExtracting HL7 v2 table CodeSystem resources …", flush=True)
