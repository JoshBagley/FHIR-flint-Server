"""
MCP-style FHIR chat — AI with tool-calling backed by PH-TS endpoints.
...
_SYSTEM_PROMPT = """\
You are a FHIR R4 terminology assistant connected to PH-TS — a public health
terminology server containing value sets, code systems, and concept maps used
by U.S. public health programs (primarily sourced from PHIN VADS / CDC).
...
# ---------------------------------------------------------------------------
# Tool execution — self-HTTP calls back into the running PH-TS API
# ---------------------------------------------------------------------------

async def _execute_tool(name: str, args: dict) -> Any:
    """Call a PH-TS endpoint and return a JSON-serialisable result."""
...
async def mcp_chat(req: McpChatRequest):
    """
    Chat endpoint with AI tool-calling backed by PH-TS FHIR endpoints.
