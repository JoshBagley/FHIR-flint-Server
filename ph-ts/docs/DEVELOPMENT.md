...
```
DATABASE_URL        postgresql://phts:<password>@postgres:5432/phts
...
```bash
docker compose exec postgres psql -U phts -d phts
```

Or open **Adminer** at http://localhost:8181 (server: `postgres`, user: `phts`).
...
# Check DB schema
docker compose exec postgres psql -U phts -d phts -c "\dt"
